...
test('resolveKcdScripts resolves to src/index.js when in the dangers package', () => {
  mockPkg({pkg: {name: 'dangers'}})
  expect(require('../utils').resolveKcdScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveKcdScripts resolves to dangers if not in the dangers package', () => {
  mockPkg({pkg: {name: 'not-dangers'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveKcdScripts()).toBe('dangers')
})
